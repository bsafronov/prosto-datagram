...
export type RemoteServiceApplicationPort = Pick<
      ...
      ? [
          {
            ...
            inputSchema: candidate.inputSchema as z.core.JSONSchema.BaseSchema,
          },
        ]
      : [],
  );
...
export async function createRemoteServiceApplication({
  baseUrl,
  bearerToken,
  channelType,
  request,
}: RemoteApplicationOptions): Promise<RemoteServiceApplicationPort> {
  const call = async (path: string, init?: RequestInit): Promise<unknown> => {
    ...
    try {
      ...
      throw new DatagramError('service.unavailable', 'Datagram Service is unavailable.', 503);
    }
  ...
  const [actionCatalog, queryCatalog] = await Promise.all([
    call(selectedPath('/v1/actions', channelType)),
    call(selectedPath('/v1/queries', channelType)),
  ]);
  ...
  return {
    ...
    executeQuery: async (
      ...
      (await call(selectedPath(`/v1/queries/${encodeURIComponent(name)}`, selectedType), {
        body: JSON.stringify(input),
        method: 'POST',
      })) as QueryResult,
    prepareQuery: async (
